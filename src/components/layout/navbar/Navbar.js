...
export default function Navbar({ currentPath = '/' }) {
  ...
  // Determine theme: Theme 1 for Doctor, Services, Appointment | Theme 2 for Home, About
          ...
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <NavbarClient theme={theme}>
              <div className="py-2 bg-[var(--color-bg-primary)]">
                <Link
                  href="/"
                  className={`relative block px-6 py-4 transition-all duration-300 group overflow-hidden text-[var(--color-text-secondary)] hover:bg-gray-50 hover:text-[var(--color-primary)] ${
                    isHomePage ? 'text-[var(--color-primary)]' : ''
                  }`}
                >
                  <span className="relative z-10 inline-block font-small">Accueil</span>
                  <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all duration-300 bg-[var(--color-primary)] ${
                ...
                <Link
                  href="/Doctor"
                  className={`relative block px-6 py-4 transition-all duration-300 group overflow-hidden text-[var(--color-text-secondary)] hover:bg-gray-50 hover:text-[var(--color-primary)] ${
                    isMedecinsPage ? 'text-[var(--color-primary)]' : ''
                  }`}
                >
                ...
                <Link
                  href="/about"
                  className={`relative block px-6 py-4 transition-all duration-300 group overflow-hidden text-[var(--color-text-secondary)] hover:bg-gray-50 hover:text-[var(--color-primary)] ${
                    isAboutPage ? 'text-[var(--color-primary)]' : ''
                  }`}
                >
