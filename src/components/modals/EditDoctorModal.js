...
export default function EditDoctorModal(props) {
  return (<div className="fixed inset-0 bg-black/80 bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div className="bg-white rounded-lg max-w-md w-full p-6">
      ...
      <div className="flex gap-3 mt-6">
        <button onClick={() => props.setShowEditModal(false)} className="flex-1 border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition">
          Annuler
        </button>
        <button onClick={props.handleEditDoctor} className="flex-1 bg-teal-700 text-white px-4 py-2 rounded-lg hover:bg-teal-800 transition">
          Enregistrer
        </button>
      </div>
    </div>
