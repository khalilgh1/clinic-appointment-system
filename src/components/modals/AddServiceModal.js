...
export default function AddServiceModal(props) {
    ...
    function addToField(field, value, clearFn) {
        if (field === 'exams') {
            // value is ignored; use category/items inputs instead
    ...
    return (
        <div className="fixed inset-0 bg-black/80 bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto relative">
                ...
                <div className="flex gap-3 mt-6 sticky bottom-0 left-6 right-6 bg-white pb-4">
                    <button onClick={() => props.setShowAddModal(false)} className="flex-1 border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition">Annuler</button>
                    <button onClick={props.handleAddService} className="flex-1 bg-primary text-white px-4 py-2 rounded-lg hover:opacity-95 transition">Créer</button>
                </div>
            </div>
        </div>
    )
